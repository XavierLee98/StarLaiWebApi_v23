...
namespace StarLaiPortal.WebApi.API.Controller
{
    ...
    public class WarehouseController : ControllerBase
    {
        ...
        public WarehouseController(IConfiguration configuration, IObjectSpaceFactory objectSpaceFactory, ISecurityProvider securityProvider)
        {
        ...
        public IActionResult Get()
        {
            try
            {
                ...
                {
                    var val = conn.Query("exec sp_getdatalist 'Warehouse'").ToList();
        ...
        public IActionResult Get(string code)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(Configuration.GetConnectionString("ConnectionString")))
                {
                    ...
                    var val = conn.Query($"exec sp_getdatalist 'Warehouse', '{json}'").ToList().FirstOrDefault();
                    return Ok(JsonConvert.SerializeObject(val, Formatting.Indented));
                }
            }
            catch (Exception ex)
