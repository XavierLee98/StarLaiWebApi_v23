...
namespace StarLaiPortal.WebApi.API.Controller
{
    ...
    public class SalesPersonController : ControllerBase
    {
        ...
        public SalesPersonController(IConfiguration configuration, IObjectSpaceFactory objectSpaceFactory, ISecurityProvider securityProvider)
        {
        ...
        public IActionResult Get()
        {
            try
            {
                ...
                {
                    var val = conn.Query("exec sp_getdatalist 'SalesPerson'").ToList();
        ...
        public IActionResult Get(string code)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(Configuration.GetConnectionString("ConnectionString")))
                {
                    ...
                    var val = conn.Query($"exec sp_getdatalist 'SalesPerson', '{json}'").ToList().FirstOrDefault();
                    return Ok(JsonConvert.SerializeObject(val, Formatting.Indented));
                }
            }
            catch (Exception ex)
